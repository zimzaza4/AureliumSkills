...
import com.archyx.aureliumskillslegacy.skills.abilities.mana_abilities.MAbility;
import com.archyx.aureliumskillslegacy.skills.abilities.mana_abilities.Treecapitator;
...
public class ForagingAbilities implements Listener {
				...
				if (event.getDamager() instanceof Player) {
					...
					if (SkillLoader.playerSkills.containsKey(player.getUniqueId())) {
						if (event.getCause().equals(DamageCause.ENTITY_ATTACK)) {
							...
							if (mat.equals(Material.DIAMOND_AXE) || mat.equals(Material.IRON_AXE) || mat.equals(Material.GOLD_AXE)
									|| mat.equals(Material.STONE_AXE) || mat.equals(Material.WOOD_AXE)) {
	...
	private void breakBlock(BlockState state, int num) {
		...
		if (matAbove.equals(Material.LOG) || matAbove.equals(Material.LOG_2)) {
	...
	public void readyTreecapitator(PlayerInteractEvent event) {
		if (Options.isEnabled(Skill.FARMING)) {
			if (event.getAction().equals(Action.RIGHT_CLICK_AIR) || event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
				Material mat = event.getPlayer().getInventory().getItemInHand().getType();
